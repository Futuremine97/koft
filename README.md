...
- `POST /api/reconstruct` — multipart: `photos[]`, `lidar`(선택), `use_lidar`, `engine`(auto|vggt|demo), `fit`, `true_length_mm`(선택)
- `GET /api/export/{result_id}/{kind}.{fmt}` — kind: foot|toes|last|shoe|insole|points, fmt: stl|obj|ply
